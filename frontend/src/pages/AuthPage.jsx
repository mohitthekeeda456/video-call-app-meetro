...
export function AuthPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (data.pendingLoginVerification) {
        setVerificationEmail(data.email);
  ...
  const verifyRegistration = async (event) => {
    ...
    try {
      const data = await api("/api/auth/verify-registration", {
        method: "POST",
        body: JSON.stringify({ email: verificationEmail || form.email, otp })
      });
      applyAuth(data);
      navigate(location.state?.from || "/", { replace: true });
    } catch (error) {
  ...
  const verifyLogin = async (event) => {
    ...
    setMessage("");
    try {
      const data = await api("/api/auth/verify-login", {
        method: "POST",
        body: JSON.stringify({ email: verificationEmail || form.email, otp })
      });
      applyAuth(data);
  ...
  return (
    <main className="grid flex-1 place-items-center py-8">
      <section className="grid w-full max-w-5xl overflow-hidden rounded-lg border border-white/70 bg-white shadow-2xl shadow-slate-950/10 md:grid-cols-[1.05fr_0.95fr]">
        <div className="flex min-h-[420px] flex-col justify-between bg-slate-950 p-8 text-white">
          <div>
            ...
            <h1 className="mt-4 max-w-md text-4xl font-black leading-tight">
              {mode === "login" ? "Welcome back to your meeting desk." : mode === "verify" ? "Verify your email to finish setup." : "Create your host account."}
            </h1>
        ...
        <form className="grid gap-4 p-6 sm:p-8" onSubmit={mode === "verify" ? (verificationPurpose === "login" ? verifyLogin : verifyRegistration) : submit}>
          <div>
            <h2 className="text-2xl font-black text-slate-950">{mode === "login" ? "Sign in" : mode === "verify" ? "Enter OTP" : "Register"}</h2>
            <p className="mt-1 text-sm text-slate-500">
              {mode === "login"
                ? "Use your existing Meetro account."
                : mode === "verify"
                  ? `Enter the 6-digit code sent to ${verificationEmail || form.email}.`
          ...
          {mode === "verify" ? (
            <input className="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-center text-2xl font-black tracking-[0.35em] outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-4 focus:ring-cyan-100" inputMode="numeric" maxLength={6} placeholder="000000" value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} />
          ) : (
            <>
          ...
          <button className="rounded-md bg-slate-950 px-5 py-3 font-black text-white transition hover:-translate-y-0.5 hover:bg-cyan-700 disabled:cursor-not-allowed disabled:opacity-60" type="submit" disabled={busy}>
            {busy ? "Working..." : mode === "login" ? "Send login OTP" : mode === "verify" ? "Verify OTP" : "Send signup OTP"}
          </button>
          <button className="rounded-md border border-slate-200 px-5 py-3 font-bold text-slate-700 transition hover:bg-slate-50" type="button" onClick={() => {
